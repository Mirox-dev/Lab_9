...
int main() {
    ...
    const auto IntGroup = GroupStats(intArr, size(intArr),
    ...
    const auto doubleGroup = GroupStats(doubleArr, size(doubleArr),
        [](const double x) {
        if (x < 0) return 0;
        if (x <= 10) return 1;
        return 2;
    ...
    const auto stringGroup = GroupStats(strArr, size(strArr),
        [](const string& x) {return x[0];},
